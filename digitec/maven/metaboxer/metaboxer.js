...
function metaboxer_admin_initialize_script() {
	...
	if ( $temp_container.length > 0 ) {
		...
		// Set the insert text
		jQuery( '.savesend' ).children( 'input[type="submit"]' ).val('Insert Attachment');
		jQuery( '.savesend' ).addClass( 'metaboxer-savesend' );
		
		// Remove fields
		jQuery( '.media-item' ).children( '.describe' ).find( 'tr.url' ).hide();
		jQuery( '.media-item' ).children( '.describe' ).find( 'tr.align' ).hide();
		jQuery( '.media-item' ).children( '.describe' ).find( 'tr.image-size' ).hide();
	}
...
function metaboxer_multiple_file_selection() {
	...
	if ( jQuery( 'form#image-form' ).length == 0 ) {
		jQuery( '.ml-submit:first' ).append( '<a class="button" id="metaboxer-attachment-select-all">'+metaboxer_vars.text_select_all+'</a> <a class="button" id="metaboxer-attachment-deselect-all">'+metaboxer_vars.text_deselect_all+'</a><br/><br/><a class="button" id="metaboxer-insert-selected">'+metaboxer_vars.text_insert_selected+'</a> <a style="text-decoration:underline;padding-left:5px;" id="metaboxer-delete-selected">'+metaboxer_vars.text_delete_selected+'</a>' );
	}
}
...
function metaboxer_admin_addEventListeners() {
	 ...
	 * @since 1.0
	 */
	jQuery( '.metaboxer-savesend input[type="submit"]' ).live('click', function(e) {
		//console.log($temp_container.length);
		//if ( $temp_container ) {
  ...
  /**
	 ...
	 * @since 1.0
	 */	
	jQuery( '#wp-content-media-buttons' ).children( 'a' ).click( function() {
		metaboxer_reset_temp_classes();
	});
